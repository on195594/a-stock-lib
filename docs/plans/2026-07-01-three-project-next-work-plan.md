...
| P1 | Phase 6 weekly PM loop | `a-stock-tracker` | 复核 weekly/daily/outcome 最新日志、`READY_CRON`、`accuracy-report`，只提交报告和状态更新 | 日志正常则维持 report-only；异常则先修行情/cron |
| P1 | Framework B label review 准备 | `a-stock-tracker` | 2026-07-26 后检查 B label 30d 已结案样本、overdue、行业覆盖、B-A delta | 满足门槛后写 review 文档；仍不直接上线 B 生产写入 |
| P1 | C 框架 checklist 提前反馈 | `a-stock-research` | 在 `cmd_checklist` 对 C 框架缺失周期标签做 warn-only 提示；B/D 仍由 `cmd_set_analysis` fail-closed | 用户在 checklist 阶段可提前看到 C 框架周期标签缺失提示 |
| P1 | parser 容错策略 spec | `a-stock-lib` → `a-stock-research` | 决定是否支持空格、全角括号等 LLM 标签变体；避免在 research 本地补临时正则 | `contracts.py` 有清晰契约和对抗测试；research 不分叉 parser |
| P1 | `watchlist-refresh` 超时保护 | `a-stock-research` | 历史 TODO 仍 open；避免单只股票分析或外部调用挂起导致整轮阻塞 | 脚本有外层 timeout/退出码测试，真实 cron 风险被隔离 |
