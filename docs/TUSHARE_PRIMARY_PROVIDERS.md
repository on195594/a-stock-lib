...
适用代码版本：`a-stock-lib==0.6.4` 候选版。生产消费者仍使用 `0.6.3`；只有在不可变 GitHub Release wheel 发布并校验后才切换到 `0.6.4`。

## 安装

候选验证必须安装本提交构建并校验过哈希的 wheel；发布前不得把 `0.6.4` 当作可用的生产 artifact：

```bash
pip install ./dist/a_stock_lib-0.6.4-py3-none-any.whl
```
