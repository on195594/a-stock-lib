...
当前版本 `0.5.1`。在 `0.5.0` 的 TuShare 行情、行业、估值、财务和分红 Provider 基线上，为 `fina_indicator` 增加收入及扣非归母净利润同比字段。`a-stock-agent-skills` runtime 已于 2026-08-23 切换到 `0.5.1`；`a-stock-tracker` 仍运行 `0.5.0`，其升级属于独立消费者 rollout。
