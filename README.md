...
当前版本 `0.6.3`：在 0.6.2 基础上增强 compact_date 空白容错并规范规则哈希缓存清理接口。`a-stock-tracker` 与 `a-stock-agent-skills` runtime 均已完成 `0.6.3` 切换和回读；Research 通过只读命令消费确定性基本面评分，tracker 继续保留既有 Framework A scorer。
