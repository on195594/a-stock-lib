...
**当前状态（2026-07-02）**：本仓库版本为 `0.2.0`。Phase 1-4 Provider 层/Provider 硬化/Tracker 切换均已完成并提交（含 BaoStock fallback 进程挂死风险隔离超时加硬）。2026-06-29 完成三轮技术债清理（P0/P1/Step A，详见 CHANGELOG）。2026-07-01 完成 `contracts.py`（FrameworkKey/FrameworkDecision/CycleStage/CycleStageAssessment/SubjectiveCategory/RatingTier/EvidenceConfidence/SubjectiveAssessment 类型 + 标签解析函数）与 `prompts/` canonical 源+渲染脚本，发布 `0.2.0`。a-stock-research 的 `cache.py` 已硬切换主观分项证据校验到 `a_stock_lib.contracts.parse_subjective_assessment_tags`（旧正则实现 `validate_subjective_evidence` 已删除）。a-stock-tracker 与 a-stock-research 均已升级并消费 `a-stock-lib==0.2.0`。全量测试 `99 passed`。**Phase 3b（周期位置判断结构化校验接线，2026-07-01同日完成）**：`prompts/` 渲染的 `AGENTS.md` 首次真正落地到 a-stock-research 并验证 codex 会自动读取（此前只验证过 agy）；`contracts.parse_cycle_stage_tag` 已接入 a-stock-research 的 `cmd_set_analysis`（不是 `cmd_checklist`——`cmd_checklist` 只对 A/C/F 框架生效，而周期位置判断必做的是 C/D/B，二者交集仅 C 一个框架，接 checklist 无法形成对 B/D 的真正约束），C/D/B 框架均为 fail-closed（B/D 原计划先设 warn-only 观察期，用户随后明确要求跳过观察期同步切至 fail-closed）。这部分代码改动全部发生在 a-stock-research 仓库（PM 直接实现+自测，未走 collab-pipeline 的 codex 派发循环），本仓库自身代码未变。2026-07-02 已把跨项目 Phase 6 weekly PM loop 在 a-stock-tracker 自动化为每周一 09:30 cron + Telegram 摘要（tracker commit `f181010`，验证 `211 passed, 1 skipped`），但 Framework B 仍保持 report-only；`cmd_checklist` 里 C 框架的 warn-only 提前反馈 UX 已在 research 侧作为 P1 完成。
